...
for idx, row in star_list.iterrows():
    ...
    trend_list = star_list[# (star_list.chip == row.chip) &
                           ...
                           (star_list.object_type == 'Star')].copy().sort_values(by='dist')[0:ntr_stars].reset_index(drop=True)
    ...
    for jj, rr in trend_list.iterrows():
        ...
        plt.scatter(tr.jd, 1- (tr.mag-tr.mag.median()), c='k')
    plt.scatter(lc.jd, lc.mag - lc.mag.median(), c='r')
    plt.show()
    cols = {}
    col_nme = []
