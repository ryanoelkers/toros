...
star_list = pd.read_csv("/Users/yuw816/Data/toros/commissioning/master/FIELD_0e.001/hold_files/"
                        + Configuration.FIELD + '_star_list_old.txt',
                        delimiter=' ',
                        header=0,
                        low_memory=False)
...
dir = "/Users/yuw816/Data/toros/commissioning/lc/FIELD_0e.001_old/"
...
for idx, row in star_list[28529:].iterrows():
    ...
    ok_data = sc(lc.raw, sigma=3)
    ok_data.mask[np.argwhere(img_stats.bd_day == 1)] = True
    ...
    trend_list = star_list[(star_list.chip == row.chip) &
                           (star_list.gc_star == 0) &
                           (star_list.dist > Configuration.APER_SIZE)].copy().sort_values(by='master_mag')[0:n_tr_stars].reset_index(drop=True)
    ...
    for idy, rw in trend_list.iterrows():
        ...
        if len(tr[tr.mag > 0]) >= len(tr[~ok_data.mask]):
            _, mdn_mag, _ = scs(tr[tr.raw > 0].raw, sigma=2)
            cols['mag_' + str(kk)] = tr.raw.to_numpy() - rw.master_mag
    ...
    plt.subplot(2, 1, 2)
    # plt.errorbar(lc[~ok_data.mask].ph, lc[~ok_data.mask].raw - lc[~ok_data.mask].trd, yerr=lc[~ok_data.mask].err, c='k', fmt='none')
    plt.scatter(lc[~ok_data.mask].ph, lc[~ok_data.mask].raw - lc[~ok_data.mask].trd, c='k')
